...
class WeatherScraper {
  ...
  private readonly requestTimeout = 10000; // 10s for edge function
  private readonly cacheMinutes = 30; // Cache data for 30 minutes
  ...
  private async getCachedData(): Promise<UnifiedWeatherData | null> {
    try {
      ...
      console.log('Found cached data from:', data.created_at);
      return data.scraped_data as UnifiedWeatherData;
    } catch (error) {
  ...
  private async cacheData(weatherData: UnifiedWeatherData): Promise<void> {
    try {
      ...
      const { error } = await this.supabase
        ...
        .upsert({
          data_hash: dataHash,
          scraped_data: weatherData,
          expires_at: expiresAt.toISOString(),
          source_status: {
            gma: weatherData.data_quality.gma_status,
            pagasa: weatherData.data_quality.pagasa_status
          },
          item_count: weatherData.weather_summary.total_items,
          has_advisories: weatherData.weather_summary.active_advisories > 0
        }, {
  ...
  async scrapePagasaWeather(): Promise<{
    ...
    try {
      ...
      for (const table of Array.from(tables)) {
        ...
        if (headersText.includes('weather condition') || 
            headersText.includes('caused by') || 
            headersText.includes('place')) {
  ...
  private async checkWeatherAdvisory(): Promise<AdvisoryStatus> {
    try {
      ...
      const noAdvisoryPatterns = [
        /no.*weather.*advisory.*issued/i,
        /no.*advisory.*issued/i,
        /no.*active.*advisory/i,
        /no.*current.*advisory/i
      ];
      
      const activeAdvisoryPatterns = [
        /weather.*advisory.*issued/i,
        /active.*advisory/i,
        /current.*advisory/i
      ];
      ...
      return {
        ...
        status: hasNoAdvisory ? "No active advisory" : 
                hasActiveAdvisory ? "Active advisory detected" : "Status unclear",
        last_checked: this.getCurrentTimestamp()
      };
  ...
  async combineWeatherData(): Promise<UnifiedWeatherData> {
    ...
    console.log('No cached data found, scraping fresh data...');
    const currentTime = this.getCurrentTimestamp();
    
    // Fetch data from both sources concurrently for better performance
    const [gmaData, pagasaData] = await Promise.all([
      this.scrapeGmaWeather(),
      this.scrapePagasaWeather()
    ]);
    ...
    console.log(`Combined data: ${gmaData.items.length} GMA items, ${pagasaData.forecast_conditions.length} PAGASA items`);
...
serve(async (req) => {
  ...
  try {
    console.log('Weather scraper starting...');
    const scraper = new WeatherScraper();
    const weatherData = await scraper.combineWeatherData();
    
    console.log('Weather scraper completed successfully');
